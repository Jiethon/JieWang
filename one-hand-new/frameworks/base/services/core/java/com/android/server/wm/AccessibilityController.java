...
final class AccessibilityController {
    ...
    public void onPreRotationChangedLocked(DisplayContent displayContent, int rotation) {
        if (mDisplayMagnifier != null) {
            mDisplayMagnifier.onPreRotationChangedLocked(displayContent, rotation);
        }
    }
    ...
    public MagnificationSpec getMagnificationSpecLocked() {
        if (mDisplayMagnifier != null) {
            return mDisplayMagnifier.getMagnificationSpecLocked();
        }
        return null;
    }
    ...
    public void setOneHandCtrlActivated(boolean activated) {
        if (mDisplayMagnifier != null) {
            mDisplayMagnifier.setOneHandCtrlActivatedLocked(activated);
        }
    }
    // END leo_liao@asus.com
    ...
    private static final class DisplayMagnifier {
        ...
        public void setMagnificationSpecLocked(MagnificationSpec spec) {
            ...
            // BEGIN leo_liao@asus.com, One-hand control
            if (spec.isOneHandControlling()) {
                mMagnifedViewport.recomputeOneHandCtrlBoundsLocked();
            } else {
        ...
        public void onWindowLayersChangedLocked() {
            ...
            // BEGIN leo_liao@asus.com, One-hand control
            if (mMagnifedViewport.isOneHandCtrlActivated()) {
                mMagnifedViewport.recomputeOneHandCtrlBoundsLocked();
            } else {
                mMagnifedViewport.recomputeBoundsLocked();
            }
        ...
        public void onPreRotationChangedLocked(DisplayContent displayContent, int rotation) {
            if (DEBUG_ROTATION) {
                Slog.i(LOG_TAG, "Pre-Rotaton: " + Surface.rotationToString(rotation)
                        + " displayId: " + displayContent.getDisplayId());
            }
            mMagnifedViewport.onPreRotationChangedLocked();
            mHandler.sendEmptyMessage(MyHandler.MESSAGE_NOTIFY_PRE_ROTATION_CHANGED);
        }
        // END leo_liao@asus.com
            ...
            if (spec != null && !spec.isNop()) {
                ...
                if (spec.isOneHandControlling()) {
                    if (!policy.isTopLevelWindow(windowType) && windowState.mAttachedWindow != null
                            && !policy.canOneHandCtrlWindow(windowType)) {
                        return null;
                    }
                    if (!policy.canOneHandCtrlWindow(windowState.mAttrs.type)) {
                        return null;
                    }
                } else {
                    if (!policy.isTopLevelWindow(windowType) && windowState.mAttachedWindow != null
        ...
        // BEGIN leo_liao@asus.com, One-hand control
        public MagnificationSpec getMagnificationSpecLocked() {
            return mMagnifedViewport.getMagnificationSpecLocked();
        }

        public void setOneHandCtrlActivatedLocked(boolean activated) {
            mMagnifedViewport.setOneHandCtrlActivatedLocked(activated);
        }
        // END leo_liao@asus.com
        ...
        private final class MagnifiedViewport {
            ...
            // BEGIN leo_liao@asus.com, One-hand control
            private boolean mOneHandCtrlActivated = false;
            // END leo_liao@asus.com
            ...
            // BEGIN leo_liao@asus.com, One-hand control
            // Fix TT-512850, the animation is broken during rotation
            public void onPreRotationChangedLocked() {
                // If we are magnifying, hide the magnified border window immediately so
                // the user does not see strange artifacts during rotation. The screenshot
                // used for rotation has already the border
                if (isMagnifyingLocked()) {
                    setMagnifiedRegionBorderShownLocked(false, false);
                }
            }
            ...
            public void setMagnifiedRegionBorderShownLocked(boolean shown, boolean animate) {
                ...
                // BEGIN leo_liao@asus.com, One-hand control
                // Always hide the viewport when one-hand control is activated
                mWindow.setShown(isOneHandCtrlActivated() ? false : shown, animate);
                // END leo_liao@asus.com
            }
            ...
            public void drawWindowIfNeededLocked() {
                // BEGIN leo_liao@asus.com, One-hand control
                if (isOneHandCtrlActivated()) {
                    recomputeOneHandCtrlBoundsLocked();
                } else {
                    recomputeBoundsLocked();
                }
                // END leo_liao@asus.com
            ...
            // BEGIN leo_liao@asus.com, One-hand control
            public void recomputeOneHandCtrlBoundsLocked() {
                mWindowManager.getDefaultDisplay().getRealSize(mTempPoint);
                final int screenWidth = mTempPoint.x;
                final int screenHeight = mTempPoint.y;
                final int offsetX = (int) mMagnificationSpec.offsetX;
                final int offsetY = (int) mMagnificationSpec.offsetY;
                final float scale = mMagnificationSpec.scale;

                mMagnificationRegion.set(0, 0, screenWidth, screenHeight);
                mMagnificationRegion.scale(scale);
                mMagnificationRegion.translate(offsetX, offsetY);

                Region nonMagnifiedBounds = mTempRegion4;
                nonMagnifiedBounds.set(0, 0, screenWidth, screenHeight);
            ...
            public boolean isOneHandControllingLocked() {
                return mMagnificationSpec.scale < 1.0f;
            }
            ...
            public void setOneHandCtrlActivatedLocked(boolean activated) {
                ...
                mOneHandCtrlActivated = activated;
            }
        ...
        private class MyHandler extends Handler {
            ...
            // BEGIN leo_liao@asus.com, One-hand control
            // Fix TT-512850, the animation is broken during rotation
            public static final int MESSAGE_NOTIFY_PRE_ROTATION_CHANGED = 6;
            // END leo_liao@asus.com
            ...
            public void handleMessage(Message message) {
                switch (message.what) {
                    ...
                    // BEGIN leo_liao@asus.com, One-hand control
                    // Fix TT-512850, the animation is broken during rotation
                    case MESSAGE_NOTIFY_PRE_ROTATION_CHANGED: {
                        final int rotation = message.arg1;
                        mCallbacks.onPreRotationChanged(rotation);
                    } break;
                    // END leo_liao@asus.com
                }
            }
