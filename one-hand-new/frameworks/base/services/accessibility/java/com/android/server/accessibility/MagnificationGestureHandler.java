...
class MagnificationGestureHandler implements EventStreamTransformation {
    ...
    private static final int STATE_ONEHAND_CTRL_DELEGATING = 5;
    private static final int STATE_ONEHAND_CTRL_PENDING_DELEGATE = 6;
    private static final int STATE_ONEHAND_CTRL_INTERACTION = 7;
    ...
    // BEGIN leo_liao@asus.com, One-hand control
    private final OneHandCtrlStateHandler mOneHandCtrlStateHandler;
    // END leo_liao@asus.com
    ...
    public MagnificationGestureHandler(Context context, AccessibilityManagerService ams,
        ...
        // BEGIN leo_liao@asus.com, One-hand control
        mMagnificationController.setMagnificationGestureHandler(this);
        mOneHandCtrlStateHandler = new OneHandCtrlStateHandler();
        // END leo_liao@asus.com
    ...
    public void onMotionEvent(MotionEvent event, MotionEvent rawEvent, int policyFlags) {
        ...
        // BEGIN leo_liao@asus.com, One-hand control
        if (mMagnificationController.isOneHandCtrlActivated()) {
            mOneHandCtrlStateHandler.onMotionEvent(event, rawEvent, policyFlags);
        } else {
        ...
        switch (mCurrentState) {
            ...
            break;
            // BEGIN leo_liao@asus.com, One-hand control
            case STATE_ONEHAND_CTRL_DELEGATING: {
                handleMotionEventStateOneHandCtrlDelegating(event, rawEvent, policyFlags);
            }
            break;
            case STATE_ONEHAND_CTRL_PENDING_DELEGATE: {
                handleMotionEventStateOneHandCtrlPendingDelegate(event, rawEvent, policyFlags);
            }
            break;
            case STATE_ONEHAND_CTRL_INTERACTION: {
                // mOneHandCtrlStateHandler handles this case
            }
            break;
            // END leo_liao@asus.com
            default: {
    ...
    public void onDestroy() {
        // BEGIN leo_liao@asus.com, One-hand control
        if (mMagnificationController.isOneHandCtrlActivated()) {
            mMagnificationController.deactivateOneHandCtrl(false);
        } else {
            mMagnificationController.setScaleAndCenter(1.0f, 0, 0, false,
    ...
    private void transitionToState(int state) {
        if (DEBUG_STATE_TRANSITIONS) {
            switch (state) {
                ...
                // BEGIN leo_liao@asus.com, One-hand control
                case STATE_ONEHAND_CTRL_DELEGATING: {
                    Slog.i(LOG_TAG, "mCurrentState: STATE_ONEHAND_CTRL_DELEGATING");
                }
                break;
                case STATE_ONEHAND_CTRL_PENDING_DELEGATE: {
                    Slog.i(LOG_TAG, "mCurrentState: STATE_ONEHAND_CTRL_PENDING_DELEGATING");
                }
                break;
                case STATE_ONEHAND_CTRL_INTERACTION: {
                    Slog.i(LOG_TAG, "mCurrentState: STATE_ONEHAND_CTRL_INTERACTION");
                }
                break;
                // END leo_liao@asus.com
                default: {
    ...
    // BEGIN leo_liao@asus.com, One-hand control
    private final class OneHandCtrlStateHandler {
        private PointerCoords[] mTempOneHandCtrlPointerCoords;
        private PointerProperties[] mTempOneHandCtrlPointerProperties;
        private Point mLastDownPosition = new Point();

        public OneHandCtrlStateHandler() {
        }

        public void onMotionEvent(MotionEvent event, MotionEvent rawEvent, int policyFlags) {
            final int action = event.getActionMasked();
            if (mCurrentState == STATE_DETECTING) {
                if (mMagnificationController.isOneHandControlling()) {
                    if (action == MotionEvent.ACTION_DOWN) {
                        if (mMagnificationController.handleTouchOnControlPanel(event)) {
                            mDetectingStateHandler.clear();
                            transitionToState(STATE_ONEHAND_CTRL_INTERACTION);
                        } else {
                            transitionToState(STATE_ONEHAND_CTRL_DELEGATING);
                        }
                        mLastDownPosition.set((int) event.getX(), (int) event.getY());
            ...
            } else if (mCurrentState == STATE_ONEHAND_CTRL_DELEGATING) {
                if (action == MotionEvent.ACTION_HOVER_EXIT) {
                    handleMotionEventStateOneHandCtrlDelegating(event, rawEvent, policyFlags);
                    transitionToState(STATE_DETECTING);
                }
            } else if (mCurrentState == STATE_ONEHAND_CTRL_INTERACTION) {
                mMagnificationController.handleTouchOnControlPanel(event);
                if (action == MotionEvent.ACTION_UP) {
                    transitionToState(STATE_DETECTING);
                }

                // Special case.
                // If touch point is moving across magnified frame,
                // then the state will transit to STATE_ONEHAND_CTRL_DELEGATING
                if (mCurrentState == STATE_ONEHAND_CTRL_PENDING_DELEGATE) {
                    MotionEvent newEvent = obtainMotionEvent(
                            event, MotionEvent.ACTION_CANCEL, false);
                    if (newEvent != null) {
                        mMagnificationController.handleTouchOnControlPanel(newEvent);
        ...
        public MotionEvent obtainMotionEvent(MotionEvent event, int newAction, boolean stickEdge) {
            final int pointerCount = event.getPointerCount();
            PointerCoords[] coords = getTempOneHandCtrlPointerCoordsWithMinSize(pointerCount);
            PointerProperties[] properties = getTempOneHandCtrlPointerPropertiesWithMinSize(pointerCount);
            final Rect magnifiedFrame = mMagnificationController.getMagnificationBounds();
            float aspectRatio = (float) magnifiedFrame.width() / magnifiedFrame.height();
            for (int i = 0; i < pointerCount; i++) {
                event.getPointerCoords(i, coords[i]);
                if (stickEdge) {
                    final float distanceX = Math.abs(coords[i].x - mLastDownPosition.x) / aspectRatio;
                    final float distanceY = Math.abs(coords[i].y - mLastDownPosition.y);
                    if (distanceX > distanceY) {
                        if (coords[i].x > mLastDownPosition.x) {
                            coords[i].x = magnifiedFrame.left;
                        } else if (coords[i].x <= mLastDownPosition.x) {
                            coords[i].x = magnifiedFrame.right - 1;
                        }
                    } else {
                        if (coords[i].y > mLastDownPosition.y) {
                            coords[i].y = magnifiedFrame.top;
                        } else if (coords[i].y <= mLastDownPosition.y) {
                            coords[i].y = magnifiedFrame.bottom - 1;
                        }
                    }
                }
                event.getPointerProperties(i, properties[i]);
            ...
            return MotionEvent.obtain(event.getDownTime(), event.getEventTime(),
                    newAction, pointerCount, properties, coords,
                    event.getMetaState(), event.getButtonState(),
                    1.0f, 1.0f, event.getDeviceId(), event.getEdgeFlags(),
                    event.getSource(), event.getFlags());
        }

        private PointerCoords[] getTempOneHandCtrlPointerCoordsWithMinSize(int size) {
            final int oldSize = (mTempOneHandCtrlPointerCoords != null)
                    ? mTempOneHandCtrlPointerCoords.length : 0;
            if (oldSize < size) {
                PointerCoords[] oldTempPointerCoords = mTempOneHandCtrlPointerCoords;
                mTempOneHandCtrlPointerCoords = new PointerCoords[size];
                if (oldTempPointerCoords != null) {
                    System.arraycopy(oldTempPointerCoords, 0,
                            mTempOneHandCtrlPointerCoords, 0,
                            oldSize);
                }
            }
            for (int i = oldSize; i < size; i++) {
                mTempOneHandCtrlPointerCoords[i] = new PointerCoords();
            }
            return mTempOneHandCtrlPointerCoords;
        }

        private PointerProperties[] getTempOneHandCtrlPointerPropertiesWithMinSize(int size) {
            final int oldSize = (mTempOneHandCtrlPointerProperties != null)
                    ? mTempOneHandCtrlPointerProperties.length : 0;
            if (oldSize < size) {
                PointerProperties[] oldTempPointerProperties = mTempOneHandCtrlPointerProperties;
                mTempOneHandCtrlPointerProperties = new PointerProperties[size];
                if (oldTempPointerProperties != null) {
                    System.arraycopy(oldTempPointerProperties, 0,
                            mTempOneHandCtrlPointerProperties, 0,
                            oldSize);
                }
            }
            for (int i = oldSize; i < size; i++) {
                mTempOneHandCtrlPointerProperties[i] = new PointerProperties();
            }
            return mTempOneHandCtrlPointerProperties;
        }
    }

    public void activateOneHandCtrl() {
        if (mMagnificationController.canActivateOneHandCtrl()) {
            if (mMagnificationController.isMagnifying()) {
                clear();
            }
            mMagnificationController.activateOneHandCtrl(false);
        }
    }

    public void deactivateOneHandCtrl() {
        if (mMagnificationController.canDeactivateOneHandCtrl()) {
            mMagnificationController.deactivateOneHandCtrl(false);
        }
    }

    public boolean dismissCling() {
        return mMagnificationController.dismissCling();
    }

    public void transitionToPendingDelegateState() {
        if (mCurrentState == STATE_ONEHAND_CTRL_INTERACTION) {
            transitionToState(STATE_ONEHAND_CTRL_PENDING_DELEGATE);
        }
    }

    private void handleMotionEventStateOneHandCtrlDelegating(MotionEvent event,
            MotionEvent rawEvent, int policyFlags) {
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                mDelegatingStateDownTime = event.getDownTime();
        ...
        if (mNext != null) {
            if (mMagnificationController.isOneHandControlling()) {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    event.getPointerCoords(i, coords[i]);
                    tempX = Math.min(magnifiedFrame.right - 1, Math.max(magnifiedFrame.left, coords[i].x));
                    tempY = Math.min(magnifiedFrame.bottom - 1, Math.max(magnifiedFrame.top, coords[i].y));
                    coords[i].x = (tempX - magnifiedFrame.left) / scale;
                    coords[i].y = (tempY - magnifiedFrame.top) / scale;
                    event.getPointerProperties(i, properties[i]);
                }
                event = MotionEvent.obtain(event.getDownTime(),
                        event.getEventTime(), event.getAction(), pointerCount, properties,
    ...
    private void handleMotionEventStateOneHandCtrlPendingDelegate(MotionEvent event,
            MotionEvent rawEvent, int policyFlags) {
        MotionEvent newEvent = mOneHandCtrlStateHandler.obtainMotionEvent(
                event, MotionEvent.ACTION_DOWN, true);
        if (newEvent != null) {
            handleMotionEventStateOneHandCtrlDelegating(newEvent, rawEvent, policyFlags);
            newEvent.recycle();
        }
        transitionToState(STATE_ONEHAND_CTRL_DELEGATING);
    }
