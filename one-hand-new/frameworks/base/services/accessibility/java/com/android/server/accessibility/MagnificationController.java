...
class MagnificationController {
    ...
    private boolean mOneHandCtrlActivated;
    private boolean mCollapseStatusBarPanels;

    private final Rect mTempRect2 = new Rect();
    

    private final float mDefaultOneHandCtrlScale;
    private final IOneHandCtrlViewport mViewport;
    private final Display mDefaultDisplay;

    private MagnificationGestureHandler mMagnificationGestureHandler;

    private final Point mTempDisplaySize = new Point();
    private final Point mTempPosition = new Point();
    ...
    public MagnificationController(Context context, AccessibilityManagerService ams, Object lock) {
        ...
        // BEGIN leo_liao@asus.com, One-hand control
        float defaultScreenSizeInch = Float.parseFloat(context.getResources().getString(
                R.string.config_oneHandCtrlDefaultScreenSizeInch));
        float deviceScreenSizeInch = Float.valueOf(SystemProperties.get(
                "ro.asus.onehand.screensize",
                IOneHandCtrlViewport.DEFAULT_ONEHAND_CTRL_SCREEN_INCH));
        mDefaultOneHandCtrlScale = (defaultScreenSizeInch > 0
                && deviceScreenSizeInch > defaultScreenSizeInch)
                        ? defaultScreenSizeInch / deviceScreenSizeInch
                        : IOneHandCtrlViewport.DEFAULT_ONEHAND_CTRL_SCALE;
        DisplayManager displayManager = (DisplayManager)
                context.getSystemService(Context.DISPLAY_SERVICE);
        mDefaultDisplay = displayManager.getDisplay(Display.DEFAULT_DISPLAY);
        mViewport = mAms.isOneHandCtrlFeatureEnabled()
                ? new OneHandCtrlViewport(
                        context,
                        new MagnificationControllerDelegate(this),
                        deviceScreenSizeInch)
                : new FakeViewport();
        // END leo_liao@asus.com
    }
    ...
    public void register() {
        synchronized (mLock) {
            if (!mRegistered) {
                ...
                // BEGIN leo_liao@asus.com, One-hand control
                mViewport.onMagnifiedBoundsChanged(mMagnificationRegion);
                // END leo_liao@asus.com
                mRegistered = true;
            }
        }
    ...
    private void onMagnificationRegionChanged(Region magnified, boolean updateSpec) {
        synchronized (mLock) {
            ...
            // BEGIN leo_liao@asus.com, One-hand control
            if (boundsChanged && isOneHandCtrlActivated()) {
                mViewport.onMagnifiedBoundsChanged(mMagnificationRegion);
            }
            // END leo_liao@asus.com
        }
    ...
    private static class SpecAnimationBridge {
        ...
        // BEGIN leo_liao@asus.com, One-hand control
        private static final int ACTION_SET_ONEHAND_CTRL_ACTIVATED = 2;
        // END leo_liao@asus.com
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case ACTION_UPDATE_SPEC:
                        ...
                        break;
                    // BEGIN leo_liao@asus.com, One-hand control
                    case ACTION_SET_ONEHAND_CTRL_ACTIVATED:
                        final boolean activated = msg.arg1 == 1;
                        setOneHandCtrlActivatedInternal(activated);
                        break;
                    // END leo_liao@asus.com
                }
            }
        ...
        private static class MagnificationSpecEvaluator
            ...
            public MagnificationSpec evaluate(float fraction, MagnificationSpec fromSpec,
                ...
                // BEGIN leo_liao@asus.com, One-hand control
                result.offsetX = Math.round(result.offsetX);
                result.offsetY = Math.round(result.offsetY);
                // END leo_liao@asus.com
                return result;
            }
        }
        ...
        public void setOneHandCtrlActivated(boolean activated) {
            if (Thread.currentThread().getId() == mMainThreadId) {
                ...
                mHandler.obtainMessage(ACTION_SET_ONEHAND_CTRL_ACTIVATED,
                        activated? 1 : 0, 0, null).sendToTarget();
            }
        }

        private void setOneHandCtrlActivatedInternal(boolean activated) {
            if (mTransformationAnimator.isRunning()) {
                mTransformationAnimator.cancel();
            }

            synchronized (mLock) {
                setOneHandCtrlActivatedLocked(activated);
            }
        }

        private void setOneHandCtrlActivatedLocked(boolean activated) {
            if (mEnabled) {
                try {
                    mWindowManager.setOneHandCtrlActivated(activated);
                } catch (Exception e) {
                }
            }
        }
    ...
    private static class WindowStateObserver
        ...
        // BEGIN leo_liao@asus.com, One-hand control
        private static final int MESSAGE_ONE_HAND_CTRL_ON_PRE_ROTATION_CHANGED = 5;
        // Fix TT-512850, the animation is broken during rotation
        private static final int MESSAGE_ONE_HAND_CTRL_SHOW_VIEWPORT_FRAME = 6;
        private static final int MESSAGE_ONE_HAND_CTRL_COLLAPSE_STATUSBAR_PANELS = 7;
        private static final int COLLAPSE_STATUSBAR_PANELS_DELAY = 300;
        // END leo_liao@asus.com
        ...
        // BEGIN leo_liao@asus.com, One-hand control
        private final StatusBarManager mSbm;
        // END leo_liao@asus.com
        ...
        public WindowStateObserver(Context context, MagnificationController controller) {
            ...
            // BEGIN leo_liao@asus.com, One-hand control
            mSbm = (StatusBarManager) context.getSystemService(
        ...
        // BEGIN leo_liao@asus.com, One-hand control
        private void handleOneHandCtrlOnRotationChanged() {
            mController.onRotationChanged();
            mHandler.removeMessages(MESSAGE_ONE_HAND_CTRL_SHOW_VIEWPORT_FRAME);
            Point displaySize = mController.getDisplayRealSize(null);
            if (displaySize.x < displaySize.y) { // portrait
                // Fix TT-523796, after rotating back to portrait, the delayed time before resuming one-hand mode is too long
                Message message = mHandler.obtainMessage(MESSAGE_ONE_HAND_CTRL_SHOW_VIEWPORT_FRAME);
        ...
        // Fix TT-512850, the animation is broken during rotation
        @Override
        public void onPreRotationChanged(int rotation) {
            mHandler.obtainMessage(
                    MESSAGE_ONE_HAND_CTRL_ON_PRE_ROTATION_CHANGED, rotation, 0).sendToTarget();
        }

        // Fix TT-512850, the animation is broken during rotation
        private void handleOneHandCtrlOnPreRotationChanged() {
            mController.onRotationChanged();
            mHandler.removeMessages(MESSAGE_ONE_HAND_CTRL_SHOW_VIEWPORT_FRAME);
        }

        private void collapseStatusBarPanels() {
            mHandler.sendEmptyMessageDelayed(MESSAGE_ONE_HAND_CTRL_COLLAPSE_STATUSBAR_PANELS,
                    COLLAPSE_STATUSBAR_PANELS_DELAY);
        }

        private void handleOneHandCtrlShowViewportFrame() {
            mController.onShowViewportFrame();
        }
        // END leo_liao@asus.com
            ...
            public void handleMessage(Message message) {
                switch (message.what) {
                    ...
                    case MESSAGE_ON_ROTATION_CHANGED: {
                        // BEGIN leo_liao@asus.com, One-hand control
                        if (mController.isOneHandCtrlActivated()) {
                            handleOneHandCtrlOnRotationChanged();
                        } else {
                    ...
                    // BEGIN leo_liao@asus.com, One-hand control
                    // Fix TT-512850, the animation is broken during rotation
                    case MESSAGE_ONE_HAND_CTRL_ON_PRE_ROTATION_CHANGED: {
                        if (mController.isOneHandCtrlActivated()) {
                            handleOneHandCtrlOnPreRotationChanged();
                        }
                    } break;
                    case MESSAGE_ONE_HAND_CTRL_SHOW_VIEWPORT_FRAME: {
                        if (mController.isOneHandCtrlActivated()) {
                            handleOneHandCtrlShowViewportFrame();
                        }
                    } break;
                    case MESSAGE_ONE_HAND_CTRL_COLLAPSE_STATUSBAR_PANELS: {
                        if (mSbm != null) {
                            mSbm.collapsePanels();
                        }
                    } break;
    ...
    // BEGIN leo_liao@asus.com, One-hand control
    public boolean isOneHandCtrlActivated() {
        return mOneHandCtrlActivated;
    }

    public boolean isOneHandControlling() {
        return isOneHandControlling(mCurrentMagnificationSpec.scale);
    }

    public boolean isOneHandControlling(float scale) {
        return scale < 1.0f;
    }

    public boolean canActivateOneHandCtrl() {
        return (mAms.isOneHandCtrlFeatureEnabled() && !mOneHandCtrlActivated);
    }

    public boolean canDeactivateOneHandCtrl() {
        return (mAms.isOneHandCtrlFeatureEnabled() && mOneHandCtrlActivated);
    }

    public void activateOneHandCtrl(boolean animate) {
        if (!canActivateOneHandCtrl()) {
            return;
        }

        mOneHandCtrlActivated = true;
        mSpecAnimationBridge.setOneHandCtrlActivated(true);

        getDisplayRealSize(mTempDisplaySize);
        if (mTempDisplaySize.x < mTempDisplaySize.y) { // portrait
            mViewport.recomputeOneHandCtrlBounds(false);
            setOneHandCtrlScaleAndPosition(
                    getPersistedOneHandCtrlScale(),
                    getPersistedOneHandCtrlPosition(),
                    animate,
                    false);
            mViewport.setFrameShown(true, animate);
            // Force collapse status bar panel
            mCollapseStatusBarPanels = true;
            collapseStatusBarPanels();
        }
    }

    public void deactivateOneHandCtrl(boolean animate) {
        if (!canDeactivateOneHandCtrl()) {
            return;
        }

        reset(animate);
        mViewport.setFrameShown(false, animate);
        mCollapseStatusBarPanels = false;
        mSpecAnimationBridge.setOneHandCtrlActivated(false);
        mOneHandCtrlActivated = false;
    }

    public boolean handleTouchOnControlPanel(MotionEvent event) {
        return mViewport.handleTouchOnControlPanel(event);
    }

    public boolean dismissCling() {
        return mViewport.dismissCling();
    }

    private void disableOneHandCtrl() {
        persistDisableOneHandCtrl();
    }

    private void setOneHandCtrlScaleAndPosition(float scale, Point position, boolean animate,
            boolean persist) {
        setOneHandCtrlScaleAndPosition(scale, position.x, position.y, animate, persist);
    }

    private void setOneHandCtrlScaleAndPosition(float scale, int x, int y, boolean animate,
            boolean persist) {
        float validScale = scale;
        int validX = x;
        int validY = y;
        if (!mViewport.isValidScale(scale)) {
            validScale = IOneHandCtrlViewport.DEFAULT_ONEHAND_CTRL_SCALE;
            validX = validY = 0; // Initial position
            Slog.i(LOG_TAG, "Invalid one-hand ctrl property, correct it"
                    + " scale: " + scale + " to " + validScale
                    + " x: " + x + " to " + validX
                    + " y: " + y + " to " + validY);
        }
        mViewport.calculateOneHandCtrlBounds(validScale, validX, validY, mTempRect2);
        mCurrentMagnificationSpec.initialize(validScale,
                mTempRect2.left,
                mTempRect2.top);
        mSpecAnimationBridge.updateSentSpec(mCurrentMagnificationSpec, animate);
        if (persist) {
            persistOneHandCtrlScale(validScale);
            persistOneHandCtrlPosition(validX, validY);
        }
    }

    public void collapseStatusBarPanels() {
        if (mCollapseStatusBarPanels) {
            mCollapseStatusBarPanels = false;
            mWindowStateObserver.collapseStatusBarPanels();
        }
    }

    public Point getDisplayRealSize(Point outSize) {
        if (outSize == null) {
            outSize = mTempDisplaySize;
        }
        mDefaultDisplay.getRealSize(outSize);
        return outSize;
    }

    public Rect getMagnificationBounds() {
        return mMagnificationBounds;
    }

    public void setMagnificationGestureHandler(MagnificationGestureHandler handler) {
        mMagnificationGestureHandler = handler;
    }

    private void persistDisableOneHandCtrl() {
        Settings.Secure.putIntForUser(mContentResolver,
                Settings.Secure.ACCESSIBILITY_ONEHAND_CTRL_ENABLED, 0,
                UserHandle.USER_CURRENT);
    }

    private void persistOneHandCtrlScale(final float scale) {
        if (mViewport.isValidScale(scale)) {
            Settings.Secure.putFloatForUser(mContentResolver,
                    Settings.Secure.ACCESSIBILITY_ONEHAND_CTRL_SCALE, scale,
                    UserHandle.USER_CURRENT);
        } else {
            Slog.i(LOG_TAG, "Invalid one-hand ctrl scale: " + scale);
        }
    }

    private float getPersistedOneHandCtrlScale() {
        return Settings.Secure.getFloatForUser(mContentResolver,
                Settings.Secure.ACCESSIBILITY_ONEHAND_CTRL_SCALE,
                mDefaultOneHandCtrlScale, UserHandle.USER_CURRENT);
    }

    private void persistOneHandCtrlPosition(int x, int y) {
        long combination = (long) x << 32 | y;
        Settings.Secure.putLongForUser(mContentResolver,
                Settings.Secure.ACCESSIBILITY_ONEHAND_CTRL_POSITION, combination,
                UserHandle.USER_CURRENT);
    }

    private Point getPersistedOneHandCtrlPosition() {
        long combination = Settings.Secure.getLongForUser(mContentResolver,
                Settings.Secure.ACCESSIBILITY_ONEHAND_CTRL_POSITION,
                0, UserHandle.USER_CURRENT);
        mTempPosition.x = (int) (combination >> 32);
        mTempPosition.y = (int) (combination & 0xffffffff);
        return mTempPosition;
    }

    private void persistDismissOneHandCtrlCling() {
        Settings.Secure.putIntForUser(mContentResolver,
                Settings.Secure.ACCESSIBILITY_ONEHAND_CTRL_CLING_DISMISSED, 1,
                UserHandle.USER_CURRENT);
    }

    private int getPersistedDismissOneHandCtrlCling() {
        return Settings.Secure.getIntForUser(mContentResolver,
                Settings.Secure.ACCESSIBILITY_ONEHAND_CTRL_CLING_DISMISSED, 0,
                UserHandle.USER_CURRENT);
    }

    private void onRotationChanged() {
        reset(false);
        mViewport.onRotationChanged(false);
    }

    private void onShowViewportFrame() {
        setOneHandCtrlScaleAndPosition(
                getPersistedOneHandCtrlScale(),
                getPersistedOneHandCtrlPosition(),
                false,
                false);
        mViewport.setFrameShown(true, false);
    }

    public class MagnificationControllerDelegate {
        MagnificationController mController;

        public MagnificationControllerDelegate(MagnificationController controller) {
            mController = controller;
        }

        public void disableOneHandCtrl() {
            mController.disableOneHandCtrl();
        }

        public void setOneHandCtrlScaleAndPosition(float scale, int x, int y, boolean animate,
                boolean persist) {
            mController.setOneHandCtrlScaleAndPosition(scale, x, y, animate, persist);
        }

        public void collapseStatusBarPanels() {
            mController.collapseStatusBarPanels();
        }

        public Point getDisplayRealSize(Point outSize) {
            return mController.getDisplayRealSize(outSize);
        }

        public void persistDismissCling() {
            persistDismissOneHandCtrlCling();
        }

        public boolean isPersistedDismissCling() {
            return getPersistedDismissOneHandCtrlCling() > 0;
        }

        public void transitionToPendingDelegateState() {
            if (mMagnificationGestureHandler != null) {
                mMagnificationGestureHandler.transitionToPendingDelegateState();
            }
        }
    }

    public interface IOneHandCtrlViewport {
        public static final String DEFAULT_ONEHAND_CTRL_SCREEN_INCH = "0";
        public static final float DEFAULT_ONEHAND_CTRL_SCALE = 0.75f;
        public static final float MAX_RESIZE_SCALE = 1.0f;
        public static final float MIN_RESIZE_SCALE = 0.65f;

        public void onMagnifiedBoundsChanged(Region bounds);
        public void onRotationChanged(boolean animate);
        public void setFrameShown(boolean shown, boolean animate);
        public void recomputeOneHandCtrlBounds(boolean animate);
        public boolean handleTouchOnControlPanel(MotionEvent event);
        public void calculateOneHandCtrlBounds(float targetScale, int x, int y,
                Rect outBounds);
        public boolean dismissCling();
        public boolean isValidScale(float targetScale);
    }

    private static final class FakeViewport implements IOneHandCtrlViewport {
        public void onMagnifiedBoundsChanged(Region bounds) {}
        public void onRotationChanged(boolean animate) {}
        public void setFrameShown(boolean shown, boolean animate) {}
        public void recomputeOneHandCtrlBounds(boolean animate) {}
        public boolean handleTouchOnControlPanel(MotionEvent event) { return false; }
        public void calculateOneHandCtrlBounds(float targetScale, int x, int y,
                Rect outBounds) {}
        public boolean dismissCling() { return false; }
        public boolean isValidScale(float targetScale) { return false; }
    }
    // END leo_liao@asus.com
}
